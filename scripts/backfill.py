...
def _already_backfilled(store: MemoryStore, project: str) -> bool:
    """Return True if we already have backfill data (significant entry count)."""
    count = store.entry_count(project)
    # If there are more than 3 entries, assume we've already run
    return count > 3
...
def backfill(
    ...
    """
    if not force and _already_backfilled(store, project):
        if verbose:
            print(f"  ~ backfill already done for '{project}' ({store.entry_count(project)} entries)")
    ...
    for commit_hash, date_str, subject in all_messages:
        ...
        if DECISION_KEYWORDS.search(subject):
            store.add_entry(
                project=project,
                entry_type="decision",
                summary=f"[ADR] {subject}",
                detail=f"from git history (commit {commit_hash[:8]})",
                tags=["decision"],
                files=[],
                created_at=f"{date_str} 12:00:00",
            )
            decisions_found += 1
...
if __name__ == "__main__":
    ...
    backfill(store, args.project, limit=args.limit, force=args.force)
